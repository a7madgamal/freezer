...
var Utils = {
    ...
    nextTick: (function () {
        ...
        var dirty = false;
        var fn;
        var hasPostMessage = !!global.postMessage &&
            typeof Window !== "undefined" &&
    ...
    findPivot: function (node) {
        ...
        if (node.__.pivot)
            return node;
        var found = 0, parents = node.__.parents, i = 0, parent;
    ...
    init: function (Frozen) {
        var commonMethods = {
            set: function (attr, value) {
                var attrs = attr, update = this.__.trans, isArray = this.constructor === Array, msg = "Freezer arrays only accept numeric attributes, given: ";
...
const specialEvents = [BEFOREALL, AFTERALL];
...
var emitterProto = {
    ...
    emit: function (eventName) {
        ...
        var listeners = this._events[eventName] || [];
        var onceListeners = [];
        var special = specialEvents.indexOf(eventName) !== -1;
        var i;
        var listener;
        var returnValue;
        var lastValue;
...
var Frozen = {
    ...
    merge: function (node, attrs) {
        ...
        var me = this, frozen = this.copyMeta(node), store = _.store, val, key, isFrozen;
        Utils.each(node, function (child, key) {
    ...
    splice: function (node, args) {
        ...
        var me = this, frozen = this.copyMeta(node), index = args[0], deleteIndex = index + args[1], child;
        // Clone the array
    ...
    transact: function (node) {
        var me = this, transacting = node.__.trans, trans;
        if (transacting)
            return transacting;
    ...
    run: function (node) {
        var me = this, trans = node.__.trans;
        if (!trans)
            return node;
    ...
    copyMeta: function (node) {
        var frozen = nodeCreator.clone(node), _ = node.__;
        Utils.addNE(frozen, {
            __: {
                store: _.store,
    ...
    refreshParents: function (oldChild, newChild) {
        var _ = oldChild.__, parents = _.parents.length, i;
    ...
    removeParent: function (node, parent) {
        var parents = node.__.parents, index = parents.indexOf(parent);
        if (index !== -1) {
    ...
    emit: function (node, eventName, param, now) {
        ...
        if (!ticking) {
            Utils.nextTick(function () {
                if (listener.ticking) {
                    var updated = listener.ticking; listener.prevState;
                    listener.ticking = 0;
...
var Freezer = function (initialValue, options) {
    ...
    var listener = frozen.getListener(), hub = {};
